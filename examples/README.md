...
This directory contains several examples to demonstrate the capabilities of the `vai` tool, from simple hot-reloading to complex, multi-job workflows

---

### 1. `simple-test`

**Purpose:** Demonstrates how to use `vai` to automatically run Go tests and inject environment variables

-   **`vai.yml`:** Configured to run `go test -v ./...` on any change to a `.go` file. It also sets the `TEST_MODE` and `DB_HOST` environment variables, which are checked in the test file
...
-   **`vai.yml`:** Uses the `series` keyword to first run `go build -o app .` and, upon success, run the compiled `./app` binary
...
-   **`vai.yml`:** A minimal configuration that runs `go run .` on any change to a `.go` file
...
-   **`vai.yml`:**
    ...
    -   A `build` job that is only run manually (e.g., `vai build`)
